...
async def test_all_entities(
    ...
    with patch("custom_components.opencode.PLATFORMS", [Platform.AI_TASK]):
        mock_config_entry.add_to_hass(hass)
        await hass.config_entries.async_setup(mock_config_entry.entry_id)
        await hass.async_block_till_done()

    assert (
        hass.states.get("ai_task.deepseek_v4_flash").state
        == "unavailable"
    )
    assert entity_registry.async_get("ai_task.deepseek_v4_flash")
...
async def test_generate_invalid_structured_data(
    ...
    with pytest.raises(
        HomeAssistantError, match="Error with OpenCode structured response"
    ):
...
async def test_generate_data_with_attachments(
    ...
    with (
        ...
        patch(
            "custom_components.opencode.entity.guess_file_type",
