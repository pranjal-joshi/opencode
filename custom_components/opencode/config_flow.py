...
class CannotConnect(HomeAssistantError):
    """Error to indicate we cannot connect to OpenCode Zen."""
...
async def _get_models(
    ...
    """Fetch the OpenAI-compatible model list from OpenCode Zen."""
    client = AsyncOpenAI(
        base_url=base_url,
        api_key=api_key,
    ...
    try:
        return [
            ...
            if is_openai_compatible_model(model.id)
        ]
...
class OpenCodeConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            try:
                await _get_models(self.hass, base_url, api_key)
            except InvalidAuth:
